...
// FIX: safer env variable handling (Railway + local mismatch safe)
const rzpKeyId = process.env.RAZORPAY_KEY_ID || process.env.Razorpay_Key_ID;
...
router.post('/order', async (req, res) => {
  ...
  const amountVal = Number(amount) || 500;
...
router.post('/verify', async (req, res) => {
  try {
    ...
    // FIX: stronger safety check for real Razorpay verification
    if (
      rzpInstance &&
      rzpKeySecret &&
      razorpay_signature &&
    ...
    const bookingRef =
      ...
      'SHIV_' + Math.random().toString(36).substring(2, 8).toUpperCase();

    const newBooking = new Booking({
      bookingId,
      customerName: bookingData.customerName || 'Valued Customer',

      // FIX: accept both possible frontend keys
      ...
      txnId:
        razorpay_payment_id ||
        'TXN_SHIV_' + Math.floor(1000000000 + Math.random() * 9000000000),
